lines = [i.replace("\n", "").split(" -> ") for i in open("05-01.txt")]

conv = [
    [list(map(int, i[0].split(","))), list(map(int, i[1].split(",")))] for i in lines
]
...
for c in range(2):
    for i in valids if c == 1 else wodiag:
        ...
        for x, y in zip(
            range(i[0][0], i[1][0] + step_x, step_x)
            if min(lh_x) != max(lh_x)
            else [min(lh_x)] * (max(lh_y) - min(lh_y) + 1),
            range(i[0][1], i[1][1] + step_y, step_y)
            if min(lh_y) != max(lh_y)
            else [min(lh_y)] * (max(lh_x) - min(lh_x) + 1),
        ):
